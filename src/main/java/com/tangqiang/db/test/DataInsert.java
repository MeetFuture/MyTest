...
public class DataInsert extends Thread {
	...
	private void insertData() {
		...
		String sInsertSql = "insert into nt_txn_" + getRandom(sArrTable) + "(C_ID,C_TERMCODE,C_CARD_NO,C_TRANS_CODE,C_C_JOURNAL_NO,C_P_JOURNAL_NO,"
		...
		for (int i = 0; i < iSent; i++) {
			...
			sbSqlSent.append(")");
			logger.info("sql:" + sbSqlSent.toString());
	...
	private void copyData(int i) {
		try {
			...
			sbSqlSent.append("insert into nt_txn_" + i + " select * from nt_txn_all;");
